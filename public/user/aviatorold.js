...
function cash_out_now(element, section_no, increment = '') {
    ...
    $('#all_bets .mCSB_container .bet_id_' + member_id + section_no + ' .column-4').html(currency_symbol + ' ' + amt);
    ...
    if (section_no == 0) {
        ...
        $(".cashout-toaster1 .out-amount").html(currency_symbol + ' ' + amt);
    ...
    if (section_no == 1) {
        ...
        $(".cashout-toaster2 .out-amount").html(currency_symbol + ' ' + amt);
        ...
        success: function (result) {
            if (result.isSuccess) {
                ...
                if (section_no == 0) {
                    ...
                    $("#my_bet_list #my_bet_section_0 .column-4").html(currency_symbol +' '+ result.data.cash_out_amount);
                    let is_main_auto_bet_checked = $("#main_auto_bet").prop('checked');
                ...
                if (section_no == 1) {
                    ...
                    $("#my_bet_list #my_bet_section_1 .column-4").html(currency_symbol +' '+ result.data.cash_out_amount);
                    let is_extra_auto_bet_checked = $("#extra_auto_bet").prop('checked');
...
function cash_out_bet(cashOutData) {
    ...
    $('#all_bets .mCSB_container .' + cashOutData.hash_id + ' .column-4').html(currency_symbol +' '+ amt);
}
...
function update_bet_list(bets, target, appendType = '') {
    ...
    for (i = 0; i < bets.length; i++) {
        ...
        if (parseFloat(bets[i].cashout_multiplier) > 0) {
            var cashOut = currency_symbol +' '+ Math.round(bets[i].cashout_multiplier*bets[i].amount);
        ...
        html += '<div class="list-items ' + isActive + ' ' + sectionNo + ' ' + '">' +
            ...
            '<div class="column-2"> <button class="btn btn-transparent previous-history d-flex align-items-center mx-auto"> ' + currency_symbol + bets[i].amount + ' </button> </div>' +
...
function update_my_new_bet(bet_amount, section_no, target) {
    ...
    html += '<div class="list-items" id="my_bet_section_' + section_no + '">' +
        ...
        '<div class="column-2"> <button class="btn btn-transparent previous-history d-flex align-items-center mx-auto fw-normal">' + currency_symbol +  parseFloat(bet_amount).toFixed(2) + '</button> </div>' +
...
function cash_out_multiplier(inc_no) {
    if (bet_array.length == 1 && bet_array[0].section_no == 0 && bet_array[0].is_bet != undefined) {
        $("#main_bet_section").find("#cash_out_amount").text(currency_symbol + ' ' + parseFloat(bet_array[0].bet_amount * inc_no).toFixed(2) );
    }

    if (bet_array.length == 1 && bet_array[0].section_no == 1 && bet_array[0].is_bet != undefined) {
        $("#extra_bet_section").find("#cash_out_amount").text(parseFloat(currency_symbol + ' ' + bet_array[0].bet_amount * inc_no).toFixed(2));
    }

    if (bet_array.length == 2) {
        $.map(bet_array, function (item, index) {
            if (item.section_no == 0 && item.is_bet != undefined) {
                $("#main_bet_section").find("#cash_out_amount").text(currency_symbol + ' ' + parseFloat(item.bet_amount * inc_no).toFixed(2));
            }
            if (item.section_no == 1 && item.is_bet != undefined) {
                $("#extra_bet_section").find("#cash_out_amount").text(currency_symbol + ' ' + parseFloat(item.bet_amount * inc_no).toFixed(2));
            }
        });
    }
}
                        ...
                        <div class="list-items bet_record_count ${result.data[i].cash_out_amount > 0 ? 'active' : ''}">
                            ...
                            <div class="column-2">
                                <button class="btn btn-transparent previous-history d-flex align-items-center mx-auto fw-normal">
                                    ${currency_symbol +' '+ result.data[i].bet_amount}
                                </button>
                            </div>
                            <div class="column-3">
                                ${multiplier}
